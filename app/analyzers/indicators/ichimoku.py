...
class Ichimoku(IndicatorUtils):
    def analyze(self, historical_data, signal=['leading_span_a', 'leading_span_b'], hot_thresh=None, cold_thresh=None):
        ...
        for index in range(tenkansen_period, ichimoku_df_size):
            ...
            last_index = index + 1
            tankansen_min = dataframe.loc[start_index:last_index-1, 'low'].min()
            tankansen_max = dataframe.loc[start_index:last_index-1, 'high'].max()
            ichimoku_values.loc[index, 'tenkansen'] = (
                tankansen_min + tankansen_max) / 2

        for index in range(kijunsen_period, ichimoku_df_size):
            start_index = index - kijunsen_period
            last_index = index + 1
            kijunsen_min = dataframe.loc[start_index:last_index-1, 'low'].min()
            kijunsen_max = dataframe.loc[start_index:last_index -
                                         1, 'high'].max()
            ichimoku_values.loc[index, 'kijunsen'] = (
                kijunsen_min + kijunsen_max) / 2

        for index in range(leading_span_b_period, ichimoku_df_size):
            start_index = index - leading_span_b_period
            last_index = index + 1
            leading_span_b_min = dataframe.loc[start_index:last_index-1, 'low'].min()
            leading_span_b_max = dataframe.loc[start_index:last_index-1, 'high'].max()
            ichimoku_values.loc[index, 'leading_span_b'] = (
        ...
        for index in range(0, ichimoku_df_size):
            span_hot = ichimoku_values.loc[ichimoku_values.index[index],
                                           'leading_span_a'] > ichimoku_values.loc[ichimoku_values.index[index], 'leading_span_b']
            close_hot = dataframe.loc[ichimoku_values.index[index],
                                      'close'] > ichimoku_values.loc[ichimoku_values.index[index], 'leading_span_a']
            if hot_thresh:
                ichimoku_values.at[ichimoku_values.index[index],
                                   'is_hot'] = span_hot and close_hot

            span_cold = ichimoku_values.loc[ichimoku_values.index[index],
                                            'leading_span_a'] < ichimoku_values.loc[ichimoku_values.index[index], 'leading_span_b']
            close_cold = dataframe.loc[ichimoku_values.index[index],
                                       'close'] < ichimoku_values.loc[ichimoku_values.index[index], 'leading_span_a']
            if cold_thresh:
                ichimoku_values.at[ichimoku_values.index[index],
