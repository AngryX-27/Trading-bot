...
class StochasticRSI(IndicatorUtils):
    def analyze(self, historical_data, period_count=14,
        ...
        for index in range(period_count, rsi_values.shape[0]):
            ...
            stoch_rsi = (
                100 * ((rsi_values.loc[rsi_values.index[index], 'rsi'] - rsi_min) / (rsi_max - rsi_min)))
            rsi_values.loc[rsi_values.index[index], 'stoch_rsi'] = stoch_rsi
        ...
        if len(rsi_values) and signal[0] in rsi_values.columns:
            if hot_thresh is not None:
                rsi_values['is_hot'] = rsi_values.loc[:,
                                                      signal[0]] < hot_thresh
            if cold_thresh is not None:
                rsi_values['is_cold'] = rsi_values.loc[:,
