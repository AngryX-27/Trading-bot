...
class CrossOver(IndicatorUtils):
    def analyze(self, key_indicator, key_signal, key_indicator_index,
        ...
        for column in new_key_indicator.columns:
            column_indexed_name = '{}_{}'.format(column, key_indicator_index)
            new_key_indicator = new_key_indicator.rename(
                columns={column: column_indexed_name})
        ...
        for column in new_crossed_indicator.columns:
            column_indexed_name = '{}_{}'.format(
                column, crossed_indicator_index)
            new_crossed_indicator = new_crossed_indicator.rename(
                columns={column: column_indexed_name})
