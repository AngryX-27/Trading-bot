...
class Notifier():
    ...
    def __init__(self, notifier_config):
        ...
        if self.discord_configured:
            self.discord_client = DiscordNotifier(
                ...
                avatar=notifier_config['discord']['optional'].get('avatar')
            )
        ...
        if self.telegram_configured:
            self.telegram_client = TelegramNotifier(
                ...
                parse_mode=notifier_config['telegram']['optional'].get(
                    'parse_mode')
            )
        ...
        if self.webhook_configured:
            self.webhook_client = WebhookNotifier(
                ...
                username=notifier_config['webhook']['optional'].get(
                    'username'),
                password=notifier_config['webhook']['optional'].get('password')
            )
    ...
    def notify_discord(self, new_analysis):
        ...
        if self.discord_configured:
            message = self._indicator_message_templater(
                new_analysis,
                self.notifier_config['discord']['optional'].get('template', '')
            )
            if message.strip():
    ...
    def notify_slack(self, new_analysis):
        ...
        if self.slack_configured:
            message = self._indicator_message_templater(
                new_analysis,
                self.notifier_config['slack']['optional'].get('template', '')
            )
            if message.strip():
    ...
    def notify_twilio(self, new_analysis):
        ...
        if self.twilio_configured:
            message = self._indicator_message_templater(
                new_analysis,
                self.notifier_config['twilio']['optional'].get('template', '')
            )
            if message.strip():
    ...
    def notify_gmail(self, new_analysis):
        ...
        if self.gmail_configured:
            message = self._indicator_message_templater(
                new_analysis,
                self.notifier_config['gmail']['optional'].get('template', '')
            )
            if message.strip():
    ...
    def notify_telegram(self, new_analysis):
        ...
        if self.telegram_configured:
            message = self._indicator_message_templater(
                new_analysis,
                self.notifier_config['telegram']['optional'].get(
                    'template', '')
            )
            if message.strip():
    ...
    def notify_webhook(self, new_analysis):
        ...
        if self.webhook_configured:
            ...
            for exchange in new_analysis:
                if exchange not in processed_analysis:
                    processed_analysis[exchange] = {}

                for market in new_analysis[exchange]:
                    if market not in processed_analysis[exchange]:
                        processed_analysis[exchange][market] = {}

                    for indicator_type in new_analysis[exchange][market]:
                        if indicator_type not in processed_analysis[exchange][market]:
                            processed_analysis[exchange][market][indicator_type] = {
                            }

                        for indicator in new_analysis[exchange][market][indicator_type]:
                            if indicator not in processed_analysis[exchange][market][indicator_type]:
                                processed_analysis[exchange][market][indicator_type][indicator] = [
                                ]
                            ...
                            for index, analysis in enumerate(new_analysis[exchange][market][indicator_type][indicator]):
                                if hasattr(analysis['result'], 'to_dict'):
                                    analysis_dict = analysis['result'].to_dict(
                                        orient='records')
                                    if analysis_dict:
                                        processed_analysis.setdefault(exchange, {}) \
                                            .setdefault(market, {}) \
                                            .setdefault(indicator_type, {}) \
                                            .setdefault(indicator, []).append(analysis_dict[-1])
                                    else:
                                        processed_analysis.setdefault(exchange, {}) \
                                            .setdefault(market, {}) \
                                            .setdefault(indicator_type, {}) \
                                            .setdefault(indicator, []).append('')
                                else:
                                    processed_analysis.setdefault(exchange, {}) \
                                        .setdefault(market, {}) \
                                        .setdefault(indicator_type, {}) \
            ...
            self.webhook_client.notify(processed_analysis)
    ...
    def notify_stdout(self, new_analysis):
        ...
        if self.stdout_configured:
            message = self._indicator_message_templater(
                new_analysis,
                self.notifier_config['stdout']['optional'].get('template', '')
            )
            if message.strip():
    ...
    def _validate_required_config(self, notifier, notifier_config):
        ...
        if notifier in notifier_config and 'required' in notifier_config[notifier]:
    ...
    def _indicator_message_templater(self, new_analysis, template):
                        ...
                        for index, analysis in enumerate(new_analysis[exchange][market][indicator_type][indicator]):
                            ...
                            if indicator_type == 'indicators':
                                for signal in analysis['config']['signal']:
                                    ...
                                    values[signal] = latest_result[signal]
                                ...
                                values[key_signal] = latest_result[key_signal]
                                if isinstance(values[key_signal], float):
                                    values[key_signal] = format(
                                        values[key_signal], '.8f')

                                values[crossed_signal] = latest_result[crossed_signal]
                            ...
                            if latest_result['is_hot'] or latest_result['is_cold']:
                                ...
                                should_alert = True
                                if analysis['config'].get('alert_frequency') == 'once':
                                    if last_status == status:
                                        should_alert = False

                                if not analysis['config'].get('alert_enabled', True):
                                    should_alert = False
        ...
        # Используем более безопасный способ объединения словарей
        self.last_analysis = self._merge_dictionaries(
            self.last_analysis, new_analysis)
        return new_message
