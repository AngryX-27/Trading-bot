...
class ExchangeInterface():
    ...
    def get_historical_data(self, market_pair, exchange, time_unit, start_date=None, max_periods=100):
        ...
        try:
            if exchange not in self.exchanges or time_unit not in self.exchanges[exchange].timeframes:
                raise ValueError(
                    "{} does not support {} timeframe for OHLCV data. Possible values are: {}".format(
                        exchange,
                        time_unit,
                        list(
                            self.exchanges[exchange].timeframes) if exchange in self.exchanges else []
                    )
                )
        ...
        if not start_date:
            ...
            if time_period not in timedelta_values:
