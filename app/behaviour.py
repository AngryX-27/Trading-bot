        ...
        for exchange in market_data:
            ...
            for market_pair in market_data[exchange]:
                ...
                if output_mode in self.output:
                    ...
                    self.logger.warning("Invalid output mode: %s", output_mode)
    ...
    def _get_crossover_results(self, new_result):
        ...
        for crossover in self.crossover_conf:
            if crossover not in crossover_dispatcher:
                self.logger.warning(
                    "No such crossover %s, skipping.", crossover)
                continue
            ...
            for crossover_conf in self.crossover_conf[crossover]:
                ...
                # Проверяем наличие всех ключей перед доступом к ним
                key_indicator_type = crossover_conf.get('key_indicator_type')
                key_indicator = crossover_conf.get('key_indicator')
                key_indicator_index = crossover_conf.get('key_indicator_index')

                crossed_indicator_type = crossover_conf.get(
                    'crossed_indicator_type')
                crossed_indicator = crossover_conf.get('crossed_indicator')
                crossed_indicator_index = crossover_conf.get(
                    'crossed_indicator_index')

                if not all([
                    key_indicator_type in new_result,
                    key_indicator in new_result.get(key_indicator_type, {}),
                    len(new_result.get(key_indicator_type, {}).get(
                        key_indicator, [])) > key_indicator_index,
                    crossed_indicator_type in new_result,
                    crossed_indicator in new_result.get(
                        crossed_indicator_type, {}),
                    len(new_result.get(crossed_indicator_type, {}).get(
                        crossed_indicator, [])) > crossed_indicator_index
                ]):
                    self.logger.warning(
                        "Invalid crossover configuration for %s, skipping.",
                        crossover
                    )
                    continue

                key_indicator_data = new_result[key_indicator_type][key_indicator][key_indicator_index]
                crossed_indicator_data = new_result[crossed_indicator_type][crossed_indicator][crossed_indicator_index]

                dispatcher_args = {
                    'key_indicator': key_indicator_data['result'],
                    'key_signal': crossover_conf['key_signal'],
                    'key_indicator_index': key_indicator_index,
                    'crossed_indicator': crossed_indicator_data['result'],
                    'crossed_signal': crossover_conf['crossed_signal'],
                    'crossed_indicator_index': crossed_indicator_index
                }
